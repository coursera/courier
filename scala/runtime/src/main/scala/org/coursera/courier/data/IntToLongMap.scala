...
final class IntToLongMap(private val dataMap: DataMap)
  ...
  override def updated[V1 >: Long](key: Int, value: V1): Map[Int, V1] = this + (key -> value)
  ...
  override def removed(key: Int): IntToLongMap = {
    val copy = dataMap.copy()
...
object IntToLongMap extends MapCompanion[IntToLongMap] {
  ...
  class DataBuilder(initial: IntToLongMap) extends mutable.Builder[(Int, Long), IntToLongMap] {
    ...
    override def addOne(kv: (Int, Long)): this.type = {
      val (key, value) = kv
  ...
  implicit def wrap(map: Map[Int, Long]): IntToLongMap = {
    IntToLongMap(map)
  }
}
