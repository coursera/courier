...
final class IntToByteStringMap(private val dataMap: DataMap)
  ...
  override def updated[V1 >: ByteString](key: Int, value: V1): Map[Int, V1] = this + (key -> value)
  ...
  override def removed(key: Int): IntToByteStringMap = {
    val copy = dataMap.copy()
...
object IntToByteStringMap extends MapCompanion[IntToByteStringMap] {
  ...
  def build(dataMap: DataMap, conversion: DataConversion): IntToByteStringMap = {
  ...
  class DataBuilder(initial: IntToByteStringMap) extends mutable.Builder[(Int, ByteString), IntToByteStringMap] {
    ...
    override def addOne(kv: (Int, ByteString)): this.type = {
      val (key, value) = kv
