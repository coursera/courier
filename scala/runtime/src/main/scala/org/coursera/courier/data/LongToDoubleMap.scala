...
final class LongToDoubleMap(private val dataMap: DataMap)
  ...
  override def updated[V1 >: Double](key: Long, value: V1): Map[Long, V1] = this + (key -> value)
  ...
  override def removed(key: Long): LongToDoubleMap = {
    val copy = dataMap.copy()
...
object LongToDoubleMap extends MapCompanion[LongToDoubleMap] {
  ...
  def build(dataMap: DataMap, conversion: DataConversion): LongToDoubleMap = {
    new LongToDoubleMap(DataTemplates.makeImmutable(dataMap, conversion))
  }
  ...
  class DataBuilder(initial: LongToDoubleMap) extends mutable.Builder[(Long, Double), LongToDoubleMap] {
    ...
    override def addOne(kv: (Long, Double)): this.type = {
      val (key, value) = kv
  ...
  implicit def wrap(map: Map[Long, Double]): LongToDoubleMap = {
    LongToDoubleMap(map)
  }
}
