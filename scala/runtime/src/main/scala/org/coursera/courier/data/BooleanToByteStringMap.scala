...
final class BooleanToByteStringMap(private val dataMap: DataMap)
  ...
  override def updated[V1 >: ByteString](key: Boolean, value: V1): Map[Boolean, V1] = this + (key -> value)
  ...
  override def removed(key: Boolean): BooleanToByteStringMap = {
    val copy = dataMap.copy()
...
object BooleanToByteStringMap extends MapCompanion[BooleanToByteStringMap] {
  ...
  def build(dataMap: DataMap, conversion: DataConversion): BooleanToByteStringMap = {
  ...
  class DataBuilder(initial: BooleanToByteStringMap) extends mutable.Builder[(Boolean, ByteString), BooleanToByteStringMap] {
    ...
    override def addOne(kv: (Boolean, ByteString)): this.type = {
      val (key, value) = kv
  ...
  implicit def wrap(map: Map[Boolean, ByteString]): BooleanToByteStringMap = {
    BooleanToByteStringMap(map)
  }
}
