...
class CrossTypeMapTest extends AssertionsForJUnit {
  ...
  @Test def booleanToStringMap_builder_addOne(): Unit = {
    ...
    builder.addOne(true -> "yes")
    builder.addOne(false -> "no")
  ...
  @Test def booleanToStringMap_builder_clear(): Unit = {
    val builder = BooleanToStringMap.newBuilder
    builder.addOne(true -> "old")
    builder.clear()
    builder.addOne(false -> "kept")
    val m = builder.result()
  ...
  @Test def intToStringMap_builder_addOne(): Unit = {
    ...
    builder.addOne(1 -> "one")
    builder.addOne(2 -> "two")
    val m = builder.result()
    assertResult(Some("one"))(m.get(1))
  ...
  @Test def intToStringMap_builder_clear(): Unit = {
    val builder = IntToStringMap.newBuilder
    builder.addOne(1 -> "old")
    builder.clear()
    builder.addOne(2 -> "new")
    val m = builder.result()
  ...
  @Test def longToStringMap_builder_addOne(): Unit = {
    ...
    builder.addOne(1L -> "one")
    builder.addOne(2L -> "two")
    val m = builder.result()
    assertResult(Some("one"))(m.get(1L))
  ...
  @Test def longToStringMap_builder_clear(): Unit = {
    val builder = LongToStringMap.newBuilder
    builder.addOne(1L -> "old")
    builder.clear()
    builder.addOne(2L -> "new")
    val m = builder.result()
  ...
  @Test def booleanToBooleanMap_builder(): Unit = {
    ...
    b.addOne(true -> false); b.addOne(false -> true)
    val m = b.result()
    assertResult(Some(false))(m.get(true))
    val b2 = BooleanToBooleanMap.newBuilder; b2.addOne(true -> true); b2.clear(); b2.addOne(false -> false)
    assertResult(None)(b2.result().get(true))
  }

  @Test def booleanToDoubleMap_builder(): Unit = {
    val b = BooleanToDoubleMap.newBuilder
    b.addOne(true -> 1.1); b.addOne(false -> 2.2)
    val m = b.result()
    assertResult(Some(1.1))(m.get(true))
    val b2 = BooleanToDoubleMap.newBuilder; b2.addOne(true -> 9.9); b2.clear(); b2.addOne(false -> 3.3)
    assertResult(None)(b2.result().get(true))
  }

  @Test def booleanToFloatMap_builder(): Unit = {
    val b = BooleanToFloatMap.newBuilder
    b.addOne(true -> 1.1f); b.addOne(false -> 2.2f)
    val m = b.result()
    assertResult(Some(1.1f))(m.get(true))
    val b2 = BooleanToFloatMap.newBuilder; b2.addOne(true -> 9.9f); b2.clear(); b2.addOne(false -> 3.3f)
    assertResult(None)(b2.result().get(true))
  }

  @Test def booleanToIntMap_builder(): Unit = {
    val b = BooleanToIntMap.newBuilder
    b.addOne(true -> 1); b.addOne(false -> 2)
    val m = b.result()
    assertResult(Some(1))(m.get(true))
    val b2 = BooleanToIntMap.newBuilder; b2.addOne(true -> 9); b2.clear(); b2.addOne(false -> 3)
    assertResult(None)(b2.result().get(true))
  }

  @Test def booleanToLongMap_builder(): Unit = {
    val b = BooleanToLongMap.newBuilder
    b.addOne(true -> 100L); b.addOne(false -> 200L)
    val m = b.result()
    assertResult(Some(100L))(m.get(true))
    val b2 = BooleanToLongMap.newBuilder; b2.addOne(true -> 999L); b2.clear(); b2.addOne(false -> 1L)
    assertResult(None)(b2.result().get(true))
  }

  @Test def booleanToByteStringMap_builder(): Unit = {
    val b = BooleanToByteStringMap.newBuilder
    b.addOne(true -> bs1); b.addOne(false -> bs2)
    val m = b.result()
    assertResult(Some(bs1))(m.get(true))
    val b2 = BooleanToByteStringMap.newBuilder; b2.addOne(true -> bs1); b2.clear(); b2.addOne(false -> bs2)
    assertResult(None)(b2.result().get(true))
  }

  @Test def intToBooleanMap_builder(): Unit = {
    val b = IntToBooleanMap.newBuilder
    b.addOne(1 -> true); b.addOne(2 -> false)
    val m = b.result()
    assertResult(Some(true))(m.get(1))
    val b2 = IntToBooleanMap.newBuilder; b2.addOne(1 -> true); b2.clear(); b2.addOne(2 -> false)
    assertResult(None)(b2.result().get(1))
  }

  @Test def intToDoubleMap_builder(): Unit = {
    val b = IntToDoubleMap.newBuilder
    b.addOne(1 -> 1.1); b.addOne(2 -> 2.2)
    val m = b.result()
    assertResult(Some(1.1))(m.get(1))
    val b2 = IntToDoubleMap.newBuilder; b2.addOne(1 -> 9.9); b2.clear(); b2.addOne(2 -> 3.3)
    assertResult(None)(b2.result().get(1))
  }

  @Test def intToFloatMap_builder(): Unit = {
    val b = IntToFloatMap.newBuilder
    b.addOne(1 -> 1.1f); b.addOne(2 -> 2.2f)
    val m = b.result()
    assertResult(Some(1.1f))(m.get(1))
    val b2 = IntToFloatMap.newBuilder; b2.addOne(1 -> 9.9f); b2.clear(); b2.addOne(2 -> 3.3f)
    assertResult(None)(b2.result().get(1))
  }

  @Test def intToIntMap_builder(): Unit = {
    val b = IntToIntMap.newBuilder
    b.addOne(1 -> 10); b.addOne(2 -> 20)
    val m = b.result()
    assertResult(Some(10))(m.get(1))
    val b2 = IntToIntMap.newBuilder; b2.addOne(1 -> 99); b2.clear(); b2.addOne(2 -> 1)
    assertResult(None)(b2.result().get(1))
  }

  @Test def intToLongMap_builder(): Unit = {
    val b = IntToLongMap.newBuilder
    b.addOne(1 -> 100L); b.addOne(2 -> 200L)
    val m = b.result()
    assertResult(Some(100L))(m.get(1))
    val b2 = IntToLongMap.newBuilder; b2.addOne(1 -> 999L); b2.clear(); b2.addOne(2 -> 1L)
    assertResult(None)(b2.result().get(1))
  }

  @Test def intToByteStringMap_builder(): Unit = {
    val b = IntToByteStringMap.newBuilder
    b.addOne(1 -> bs1); b.addOne(2 -> bs2)
    val m = b.result()
    assertResult(Some(bs1))(m.get(1))
    val b2 = IntToByteStringMap.newBuilder; b2.addOne(1 -> bs1); b2.clear(); b2.addOne(2 -> bs2)
    assertResult(None)(b2.result().get(1))
  }

  @Test def longToBooleanMap_builder(): Unit = {
    val b = LongToBooleanMap.newBuilder
    b.addOne(1L -> true); b.addOne(2L -> false)
    val m = b.result()
    assertResult(Some(true))(m.get(1L))
    val b2 = LongToBooleanMap.newBuilder; b2.addOne(1L -> true); b2.clear(); b2.addOne(2L -> false)
    assertResult(None)(b2.result().get(1L))
  }

  @Test def longToDoubleMap_builder(): Unit = {
    val b = LongToDoubleMap.newBuilder
    b.addOne(1L -> 1.1); b.addOne(2L -> 2.2)
    val m = b.result()
    assertResult(Some(1.1))(m.get(1L))
    val b2 = LongToDoubleMap.newBuilder; b2.addOne(1L -> 9.9); b2.clear(); b2.addOne(2L -> 3.3)
    assertResult(None)(b2.result().get(1L))
  }

  @Test def longToFloatMap_builder(): Unit = {
    val b = LongToFloatMap.newBuilder
    b.addOne(1L -> 1.1f); b.addOne(2L -> 2.2f)
    val m = b.result()
    assertResult(Some(1.1f))(m.get(1L))
    val b2 = LongToFloatMap.newBuilder; b2.addOne(1L -> 9.9f); b2.clear(); b2.addOne(2L -> 3.3f)
    assertResult(None)(b2.result().get(1L))
  }

  @Test def longToIntMap_builder(): Unit = {
    val b = LongToIntMap.newBuilder
    b.addOne(1L -> 10); b.addOne(2L -> 20)
    val m = b.result()
    assertResult(Some(10))(m.get(1L))
    val b2 = LongToIntMap.newBuilder; b2.addOne(1L -> 99); b2.clear(); b2.addOne(2L -> 1)
    assertResult(None)(b2.result().get(1L))
  }

  @Test def longToLongMap_builder(): Unit = {
    val b = LongToLongMap.newBuilder
    b.addOne(1L -> 100L); b.addOne(2L -> 200L)
    val m = b.result()
    assertResult(Some(100L))(m.get(1L))
    val b2 = LongToLongMap.newBuilder; b2.addOne(1L -> 999L); b2.clear(); b2.addOne(2L -> 1L)
    assertResult(None)(b2.result().get(1L))
  }

  @Test def longToByteStringMap_builder(): Unit = {
    val b = LongToByteStringMap.newBuilder
    b.addOne(1L -> bs1); b.addOne(2L -> bs2)
    val m = b.result()
    assertResult(Some(bs1))(m.get(1L))
    val b2 = LongToByteStringMap.newBuilder; b2.addOne(1L -> bs1); b2.clear(); b2.addOne(2L -> bs2)
    assertResult(None)(b2.result().get(1L))
  }
}
