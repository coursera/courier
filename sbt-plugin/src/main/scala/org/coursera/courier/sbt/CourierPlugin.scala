...
import CourierExceptionCompat._
 ...
 * @author Joe Betz
 */
object CourierPlugin extends AutoPlugin with CourierPluginCompat {
  ...
  private[this] def pegasusArtifacts(conf: Configuration): Seq[Def.Setting[_]] = {
    ...
    Defaults.packageTaskSettings(packageDataModel, packageDataModelMappings) ++
      ...
      Seq(
        packagedArtifacts ++= { Classpaths.packaged(Seq(packageDataModel)) }.value,
        artifacts ++= { Classpaths.artifactDefs(Seq(packageDataModel)) }.value,
        ivyConfigurations ++= List(dataTemplateConfig, defaultConfig),
        artifact in (Compile, packageBin) ~= { (art: Artifact) =>
          cloneArtifact(art, art.name, art.configurations ++ Vector(dataTemplateArtifactConfig))
        }
      )
  }
