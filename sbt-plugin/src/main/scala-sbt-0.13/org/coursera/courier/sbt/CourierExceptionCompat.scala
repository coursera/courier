...
object CourierExceptionCompat {
  ...
  class CourierCompilationProblem(
    ...
    def position(): Position =
      new CourierCompilationErrorPosition(source, atLine, column)
  }
  ...
  class CourierCompilationErrorPosition(
    ...
    def toMaybe[T](option: Option[T]) =
      option
        .map { optionValue =>
          new Just(optionValue)
        }
        .getOrElse(new Nothing[T])

    def line(): Maybe[Integer] =
      toMaybe(atLine.map(_.asInstanceOf[java.lang.Integer]))
    def lineContent(): String = ""
    def offset(): Maybe[Integer] =
      toMaybe(column.map(_.asInstanceOf[java.lang.Integer]))
  ...
  class Nothing[T] extends Maybe[T] {
    ...
    def get(): T =
      throw new IllegalStateException("Cannot call get() on a Maybe that is Nothing.")
  }
