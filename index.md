...
Building internet applications requires messaging structured data between servers and clients.  Mobile development is often done in Swift, Objective-C and Java, web development in Javascript, and server-side development in another language, such as Scala.
...
In schema driven data modeling, data bindings are generated from the schema for each language. Developers then code against those data bindings and get a strong guarentee that the JSON that those bindings read and write are compatible with all the other data bindings generated for the other languages from the same schemas. 
