...
The data bindings generated by Courier work with all the Codecs that have been built for Pegasus. Since Pegasus includes Codecs for JSON as well as multiple binary protocols (Avro, PSON, BSON, ...), developers can easily switch between protocols as needed, or even write their own.

This means developers are not locked into using the same data protocol they use in their REST APIs as they use to persist data in a document store.

For example, Courier's ability to read and write using a variety of codecs makes this trivial to:
